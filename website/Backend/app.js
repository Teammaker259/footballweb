...
app.get("/", (req, res) => {
  res.send("你好，这里是football game的后端数据接口");
});
...
//       path:'/news',
//       handler: (request, h) => (
//           ScrapingHandler.getFootballNews()
//       )
...
app.get('/news', async (req, res) => {
  try {
    const footballNews = await ScrapingHandler.getFootballNews(); // 请确保ScrapingHandler是可用的
    res.json(footballNews);
  } catch (error) {
    console.error(error);
